...
class AgSectionFileCreator(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSource(
                self.INPUT,
                self.tr('Input layer with quadrats'),
        ...
        with open(file, "w") as output_file:
            for current, feature in enumerate(features):
                ...
                if feature.hasGeometry():
                    ...
                    s = self.convertWGS84ToLocal(verticeList[0].y(), verticeList[0].x())
                    output_file.write(s +"\n")
                    self.count = self.count + 1
                    
                    s = self.convertWGS84ToLocal(verticeList[3].y(), verticeList[3].x())
                    output_file.write(s +"\n")
                    self.count = self.count + 1
                    
                    s = self.convertWGS84ToLocal(verticeList[1].y(), verticeList[1].x())
                    output_file.write(s +"\n")
                    self.count = self.count + 1
                    
                    s = self.convertWGS84ToLocal(verticeList[2].y(), verticeList[2].x())
                    output_file.write(s +"\n")
                    self.count = self.count + 1
